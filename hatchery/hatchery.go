...
func launch(w http.ResponseWriter, r *http.Request) {
	...
	if payModel == nil || payModel.Local {
		...
		Config.Logger.Infow("Launching ECS workspace",
			"username", userName,
	...
	if err != nil {
		Config.Logger.Errorw("Error during launch",
			"error", err.Error(),
			"username", userName,
			"paymodel", payModel,
		)
...
func terminate(w http.ResponseWriter, r *http.Request) {
	...
	Config.Logger.Infow("Terminating workspace",
		"username", userName,
	)
	if payModel != nil && payModel.Ecs {
		...
		if err != nil {
			...
			Config.Logger.Infow("Succesfully terminated all resources related to ECS workspace",
				"username", userName,
			)
		...
		Config.Logger.Infow("Terminated workspace",
			"username", userName,
			"paymodel", payModel,
		)
...
func createECSCluster(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		Config.Logger.Errorw("Error creating ECS cluster",
			"error", err.Error(),
			"username", userName,
			"paymodel", payModel,
		)
...
func statusEcs(ctx context.Context, userName string, accessToken string, awsAcctID string) (*WorkspaceStatus, error) {
	...
	if err != nil {
		Config.Logger.Errorw("Error getting status of ECS workspace",
			"error", err.Error(),
			"username", userName,
			"awsAcctID", awsAcctID,
		)
...
func launchEcsWorkspaceWrapper(userName string, hash string, accessToken string, payModel PayModel) {
	...
	if err != nil {
		Config.Logger.Errorw("Error launching ECS workspace",
			"error", err.Error(),
			"username", userName,
			"paymodel", payModel,
		)
		...
		if err != nil {
			Config.Logger.Errorw("Error terminating ECS workspace",
				"error", err.Error(),
				"username", userName,
				"paymodel", payModel,
			)
